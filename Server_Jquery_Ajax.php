 ...
 */
    ...
    if ($customer_id_product != null) {
        $query = "SELECT p.produit_id, p.itemname, p.itemdescription, p.regularprice, p.uomprice, p.quantitetotal, p.rabais, p.regularprice1, p.regularprice2, p.regularprice3, p.regularprice4, p.regularprice5, p.uomprice1, p.uomprice2, p.uomprice3, p.uomprice4, p.uomprice4, c.isRabais, c.customer_type, c.discount_type, c.discount_level, c.price_list, u1.description FROM produit p, customers c, unit1 u1 WHERE p.produit_id=$id AND c.customers_id=$customer_id_product AND u1.unit_id=1";
        $fetch_data = mysqli_query($con, $query);
        $row = mysqli_num_rows($fetch_data);
        if ($row > 0) {
            while ($row = mysqli_fetch_assoc($fetch_data)) {
